...
## The words and marks the index prints itself

`see` and `see also` are the extension's own words, and so is the `Symbols`
heading over the entries that file under no letter. So is the punctuation an
entry carries: the mark in front of an entry's locators and between one locator
and the next — and in front of a cross-reference where no other mark has been
printed yet, which is what an entry that is only a `see=` prints — and the mark
between two cross-references. Set any of the five
under `index-labels:`, whose keys are `symbols`, `see`, `see-also`, `separator`
and `xref-separator`:
...
A separator key sets the glyph alone. The space after it is the extension's
own, written whatever you set, so an entry can never come out with its locator
run into the term in front of it.

The Arabic marks above are what these two keys exist for — no language table
supplies them, so a document wanting them writes them here. Setting them does
not make a right-to-left index come out right on its own: that is a known
limitation of this extension, and a `lang: ar` document meets it whatever it
writes under `index-labels:`.

The map goes at a document's top level, covering every index it has, or inside
one entry of `indexes:`, covering that index alone; where both name a key, the
index's own wins key by key, and a key written in neither place keeps its
English default. It is `index-labels:` rather than `labels:` because `labels:`
at a document's top level is Quarto's own map and this extension leaves it
alone.

A value a reader could not read is refused rather than printed. Four shapes
are refused: an empty value; a value made only of characters that print
nothing, such as a non-breaking space, a zero-width space or one of the
typographic spaces; a value written as a nested map; and one written as a list,
the last two being what over-indenting the map by a level produces. Each is
reported where it was written, and the key then falls back the way a key you
never wrote does — to the document's top-level map where an index's own wrote
it, then to your `lang:` for the three words, and then to the English word or,
for the two marks, which follow no language at all, straight back to `,` and
`;`. An `index-labels:` written inside an `indexes:` entry that is itself
refused as a declaration sets no word either, and that is reported beside the
reason the entry was refused.

Set them only where the extension's own choice is wrong for you. Left alone
they follow your document's `lang:` — Spanish, French, German and Italian are
covered — so a `lang: fr` document prints `voir` and `voir aussi` with nothing
written under `index-labels:` at all, and a language the extension does not
cover keeps the English words.

This reaches the HTML and EPUB indexes only: a LaTeX index gets the two words
from babel, which sets them from your document's `lang:`, and takes its
punctuation from `makeindex` and from the LaTeX this extension emits around
`\see` and `\seealso`, which neither separator key touches.
