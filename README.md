...
A fifth report belongs to PDF alone. The index tool stores three levels, and
this extension folds a deeper entry into its third rather than lose it, so two
entries written at different depths can end up printing at one place — and each
still files under the sort key it was written with, which stores the entry
twice and prints it twice, in two places, identically. Both keys and the
printed path they contest are named, because which of the two you meant is
yours to choose. The HTML index applies no such ceiling, so there the two
entries are two and nothing is reported.
