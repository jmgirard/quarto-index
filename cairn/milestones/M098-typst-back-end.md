...
- 2026-09-13: T4 (commit after e809c61): `examples/typst-index.qmd`, the manifests `tests/typst-index-main.tsv` and `tests/typst-index-people.tsv`, the reader `tests/typstindex.py` and the AC1-AC3 section with 15 plants and a clean control, each plant red with its own row. `tests/pdfindex.py` is unchanged: it reads the Typst index's order, levels and footer as they stand. The reader takes links from the PDF's link annotations, because `pdftohtml -xml` at its default zoom assigned a link to the wrong characters.
