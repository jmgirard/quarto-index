...
- [ ] T3. `recover_record(ctx, file)`: read and `pandoc.read` the chapter
...
- [ ] T4. Call it from `store_read`'s unusable branches and from none of the
      absent branch; teach the HTML locator to emit a bare page href where a
      mark carries no anchor.
- [ ] T5. Recovered markers reach `marker_chapter`, so `placing` and `first`
      are settled from them.
- [ ] T6. The record-unreadable and record-stale reports gain a clause saying
