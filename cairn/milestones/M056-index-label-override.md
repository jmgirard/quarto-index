...
- 2026-08-29: review — the three fix-now items landed and both suites were re-run green over the final tree (441 and 863 checks, both exit 0). All eight criteria now carry fresh evidence in the Review section. Pre-gate checkpoint.
...
- **AC7 — pass.** Read of the three site pages against the list.
  ...
  is Quarto's own.
- **AC8 — pass.** After the three fix-now items landed, both commands were
  re-run sequentially over the final tree: `tests/run-tests.sh` exit 0 at 441
  checks, `tests/run-tests.sh --self-test` exit 0 at 863 checks. (An earlier
  pair of runs over the pre-fix tree gave the same two figures and exit codes.)
