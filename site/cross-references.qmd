...
## The words in front of a target

`see` and `see also` are the extension's own words, and so is the `Symbols`
heading over the entries that file under no letter. Set any of the three under
`index-labels:`, whose keys are `symbols`, `see` and `see-also`:
...
from babel, which sets them from your document's `lang:`.
