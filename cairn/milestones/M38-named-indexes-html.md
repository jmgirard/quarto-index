...
- [ ] T2: Read `index=` on a mark in all four Span passes
      ...
      default index.
- [ ] T3: Key the format-neutral accumulators per index — `marked_paths`,
...
- [ ] T4: Make `resolve_markers` and `place_index` in `marker.lua` per index:
...
- [ ] T5: `html.lua`: build one entry tree per index and emit one section
...
- [ ] T6: `latex.lua`/`index.lua` and `book.lua`: file every named-index mark
