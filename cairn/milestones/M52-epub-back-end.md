...
- 2026-08-28: two items raised by the maintainer at the gate, both outside this milestone and both to be filed at the post-merge hygiene pass, where the terminal-row prune frees the ROADMAP line the caps leave no room for today (59 of <60 lines, 23,756 of <24,000 bytes). (a) Five GitHub Actions are behind by one to four majors — `actions/checkout` v4 against v7.0.1, `upload-artifact` v4 against v7.0.1, `download-artifact` v4 against v8.0.1, `upload-pages-artifact` v3 against v5.0.0, `deploy-pages` v4 against v5.0.0; `quarto-dev/quarto-actions/setup` v2 is current. A re-pin of five dependencies, so a question gate and a D-entry, and it touches D-024's major-tag policy: its own work, not this branch's. (b) One `render (floor, 1.4.549)` leg failed at 19:50 UTC and did not reproduce — the book fixture's HTML index placed at `sub/two.qmd`'s marker rather than `last.qmd`'s, leaving `last.html` with no section to extract; identical code passed at 19:39, 19:42 and 19:57. Filed against the existing version-portability row rather than investigated, at the maintainer's selection.
