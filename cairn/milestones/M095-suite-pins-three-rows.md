...
- 2026-09-11: the claim audit returned three corrections. Two landed in `tests/stateprobe.py`: the indexes probe keeps the `read(doc.meta)` call as well as the two installation lines, and the `index_labels` reason names the unnamed cell no declaration can name. The third, how a chapter numbers the anchors it mints, waits for the running suite to end, since editing `tests/run-tests.sh` during a run corrupts that run. The audit's own line follows the re-read.
