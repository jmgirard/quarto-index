...
- 2026-09-05: review round 3. All six criteria green with fresh evidence on this tree and every box ticked against a recorded evidence line: the captured render carries 99 ids and no repeat where the merge-base repeats 12, the publication is clean where the merge-base fails 22 clauses across 6 documents, 12 refusals name their term and id, and the whole suite is 1412 checks, exit 0. cairn_validate clean. Three lenses returned 7 findings, one lens each contributing zero. X1, X4 and X5 were reproduced by driving the census walker itself: a `script` or `style` element in a raw HTML block aborts the walk, because the closing tag is re-read as an opening one, so every `id=` after it in that block goes uncounted and a mark written with one keeps a contested id in silence — the class the Goal forbids, introduced by T13 and invisible to a fixture that writes each case in a block of its own. It falsifies no criterion: AC1 quantifies over the fixture, not over a rendered page. Put to the maintainer with the thrash context — a return here would have been defect return 3.

- 2026-09-05: gate decision — descope, then merge. No fourth round is convened. The residue is recorded rather than fixed: KI254 (the census walk's three wrong shapes, X1/X4/X5) and KI255 (writer-generated ids, X2) are new `DESIGN.md` known issues, and `CHANGELOG.md` and `site/html.qmd` now state the two names the census does not see, held by two new claim rows (12 -> 14). X3 and X6 were corrected in place. The code fixes and the suite's untested census domain exit to candidate rows in the hygiene pass. Defect returns stay at 2.
- 2026-09-06: step-7 approval: PR #79 approved for merge.
