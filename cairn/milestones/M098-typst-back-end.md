...
- [ ] T5: Add the Typst checks for `examples/named-indexes.qmd` and `examples/book/`, with their manifests and plants. Add `author:` to `examples/book/_quarto.yml` and run the existing book checks. Add the Typst checks and manifests for the four escaping and Unicode fixtures and the two label fixtures in AC8. Every character Typst reads as markup in a term, a sort key or a label is escaped in T3.
- [ ] T6: Add the Typst step to the `pdf` job of `versions.yml`, which already installs poppler. Start the workflow by hand and record the run URL.
