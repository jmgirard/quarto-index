...
- 2026-09-01: review gate, user chose fix-now for F1-F3, F6, F8-F10: three claim rows added (the five-things lead, the include/cell bullet, the unreadable-source bullet) and the topic-sentence row replaced by the `entry=`/`sort=`/`see=` sentence, 21 rows, the count-coupled string moved 18 to 21; the counts loop bounds the ordinal before expanding it; the backstop names the counts file, the perl death message no longer says "did not compile", and the two slipped expressions carry `{slipped}` for a replacement. Isolated re-runs: 21 rows stated on the page, the copy missing the include bullet now red naming that row; the mutant probes unchanged (intact 1/1, each slip named). Both suite legs re-run below.
...
- AC3/AC4 re-verified after the gate fixes: the 21 rows extracted from the heredoc state all 21 on `site/books.qmd`, exit 0, and the unheld include bullet's deletion is now red; `tests/run-tests.sh` 578 checks exit 0, then `tests/run-tests.sh --self-test` 1071 checks exit 0, sequential, both `M066-AC2 self-test` lines printing.
