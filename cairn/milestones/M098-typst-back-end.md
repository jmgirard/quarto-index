...
- [x] T8: Write a hand-derived manifest of the terms `examples/xref-escaping.qmd` prints. Hold the AC8 `typstcheck.py source` derivation for that fixture to it, as for the other three fixtures. Add a plant that is red on a changed derived term. (Review finding 5.)
- [x] T9: Make the AC5 book check compare the page numbers of a hand-derived manifest, with the pages fixed from the fixture source. It no longer reads chapter bounds from the PDF under test. Add a plant that moves a locator one page inside its chapter and is red. (Review finding 4.)
...
- 2026-09-13: T12 code: the suite runs `typstindex.py pages` green on both indexes of the AC1 render, beside the red pageplus-pages plant. The `run-tests.sh` comment above `m064_mutant_html` now names `html.lua` for the HTML locator rule and `entries.lua` for the tree. The CHANGELOG entry states the T10 rule, which the nocover plant backs.
...
Sync: `origin/main` has not moved and `main` has no unpushed commits. The suite ran with `--self-test` at fd9c364 on Quarto 1.10.18: exit 0, 1607 checks passed. All 28 M098 plants were red on their own failure.

- AC1: verified. The fixture declares two indexes and no `lang:`, and it carries the ten forms with fixed page breaks. `Index of Terms` (26 lines, `kelp` included) and `Index of People` (52 lines) match the tracked manifests, each locator's page included. The people index fills both columns.
- AC2: verified. The AC2 case check finds each case row in the manifest. The reader matched each row, merged same-page marks and ranges included, and the bold shared page and no other bold. Plants nomerge, norange, samepage, nobold, sharedpage, allbold and nocover are red.
- AC3: verified. `Index of Terms` prints 15 locators on page 5 with exactly 15 links, and `Index of People` prints 26 locators on page 6 with 26 links. Each link target matched the manifest page, the opening page for a range. Plants closelink and xreflink are red.
- AC4: verified. The `main` index (19 lines) and the `authors` index (20 lines) of `examples/named-indexes.qmd` match their hand manifests at every level. The 6 placement lines are in reading order. Plants named-fold and named-unplaced are red.
- AC5: verified. The Typst book is one PDF with `Index of Subjects`, `Index of People` and `Index of Places`. The order check reads main and people at the `last.qmd` markers, with places after them. `typstcheck.py book` compares each locator's printed page with hand-stated pages (25, 2 and 2 lines). It holds the four chapter headings to pages 5, 7, 9 and 11, and it takes no expected number from the PDF. Shared Term prints 5, 7 and 9. The HTML, PDF and EPUB book checks pass. Plants book-noclose, book-unplaced and book-pageplus are red.
- AC6: verified. The manually started run 34791155718 at 0c72455 passed both Typst steps on the floor leg (1.5.52), the pinned leg (1.10.18) and the release leg. The steps render `examples/typst-index.qmd` and read both manifests with `typstindex.py pages`. `git diff --name-only 0c72455 HEAD` lists only `cairn/` files, so the run is on the tree under review.
- AC7: verified. README.md:5, `site/index.qmd:9` and `site/back-end-differences.qmd:7` state four back-ends, and the `lang:` item (line 62 on main, now 74) says "All four". Each of the ten numbered items covers Typst. The sweep grep gives 17 hits, the count the work log names with each correction. `site/other-formats.qmd` names no Typst. `typst.qmd` is in `site/_quarto.yml` and linked from `site/output.qmd`, and it states two columns, no Typst package and Quarto 1.10.18. The CHANGELOG `## Unreleased` entry states the back-end. The M098-AC7 claim checks (28 page claims, 6 CHANGELOG claims) and the T7 and T11 plants passed.
- AC8: verified. Typst renders of the four escaping and Unicode fixtures exit 0, and every printed term matches its source's derivation in NFC. The derivation is held to a hand statement for each of the four fixtures: 94, 643, 94 and 8 entries. The two label fixtures print the Spanish words and the `index-labels:` override words. Plants esc-backslash, es-english and xref-changed are red.

Consistency gate: `cairn_validate.py` exit 0, all checks passed, with two advisories (8 criteria and 13 tasks over their split tripwires). No DESIGN principle changed, so `cairn_impact` was skipped. The generic profile names no toolchain checks.

Independent review, three fresh lenses. The [S] prior-review lens found no regression: first-pass findings 1, 2, 4, 5, 8 and 11 are fixed, and 3, 6, 7, 9 and 10 are recorded where the triage put them. The PR comment probe returned none. The [S] blame-history lens found no conflict with past intent. It noted, unranked, that the fold of a page just after a range is disclosed only in a `typst.lua` comment. The [O] diff-bug lens found AC1 to AC8 met as written and gave 6 findings, ranked:

1. `typst.lua:242-258`: a mark in a figure caption is recorded twice, because Quarto copies the caption into the image alt text. A range opened there warns that the range is already open. Checked on main's extension: the same warning appears in an HTML render before M098, so the diff did not introduce it.
2. `site/back-end-differences.qmd:47-54`, `site/typst.qmd:49-53`: makeindex folds a page just after a range into the range (`kelp, 2–4` in PDF), and Typst prints `2–3, 4`. Only a `typst.lua` comment states it.
