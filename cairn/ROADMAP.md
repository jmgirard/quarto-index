...
- Marker hardening: a nested marker leaves its container empty, and the marker class on a non-div (span, heading) is ignored with no warning — added 2026-08-17 — M04 review F6/F7; narrowed 2026-08-18 by M08, which reports both, except that a table cell emptied by a nested marker is still stripped unreported (divs, block quotes, figures, footnotes, list items and definitions are covered), and a marker class written in `subtitle:` or `abstract:` — which Quarto renders — is reported nowhere, the report reading the document's blocks alone (M08 review F3/R3/R4, partial fix)
