...
- 2026-09-03: /milestone-review started; `main` unmoved; draft PR #71 opened. Fresh `--self-test` run exit 0 (1228 checks); AC1–AC4 ticked against the evidence in Review; plain run in progress for AC5. `cairn_validate` exit 0.
- 2026-09-03: plain run exit 0 (654 checks); AC5 ticked. Gate clean. Review F1 (a `title:` mark reflected into the navigation envelope) reproduced on the branch and on `main`.
- 2026-09-03: deviation — the [O] diff-bug reviewer failed four times on server-side errors (529, 529, 500, 529 from the Opus endpoint) before reporting; the lens re-run fresh-context on Sonnet, logged here and named at the gate. The [S] blame-history and [S] prior-review lenses ran on Sonnet as specified and each reported zero findings.

## Decisions

## Review

Reviewed 2026-09-03 on the branch at `7006e17`, `main` unmoved since the branch was cut (`git fetch`; 0 ahead / 0 behind). Draft PR #71.

- AC1 — verified. Fresh `tests/run-tests.sh --self-test` run (exit 0, 1228 checks): the record leg renders `six.qmd`, `seven.qmd` and `eight.Rmd` so each writes its own record, then `index.qmd`; `check_index_sections` matched every row of `M070_SECTIONS_RECORDED` in order — `Hasp` at `six.html`, `Mullion`, `Nacelle` and `Lanyard` at `eight.html`, one locator each with no fragment, the rows the cold and listed-unopenable legs print for those four terms; `Ingot` under key `Az` at `seven.html` then `seven.html#qi-mark-1`. Three planted defects went red against the same manifest: the reflected copies' class removal taken out (`Hasp` prints its page then two fragments into ids `six.html` lacks), the page locator turned back into an anchor (`Hasp` prints one fragment locator), and the forgery strip taken out with a forged tag on `seven.qmd`'s body mark (`Ingot` loses its fragment), each beside its passing control.
- AC2 — verified. Same run: `tests/fragments.py resolve` walked every locator of every index section in the three `book-extensions` captures (cold, listed-unopenable, record) and the `front-matter` capture and read the ids of each page an href with a fragment names; every fragment resolved. The reader was shown red on a capture with one href rewritten to a fragment its page does not carry, on one rewritten to a page the capture lacks, and on a page with no index section (empty domain), each failure naming its target.
- AC3 — verified. Same run: `examples/front-matter.qmd` rendered to HTML with zero extension warnings; its index matched all 9 rows of `M071_FRONT_MATTER_SECTIONS` — `Bollard #qi-mark-1`, `Halyard #qi-mark-2`, `Gimbal #qi-mark-3`, `Capstan #qi-mark-4`; `fragments.py inside` found `qi-mark-1..3` each once inside `#title-block-header` and `outside` found `qi-mark-4` once outside it; the containment reader was shown red asked the wrong way round for each of the four ids.
- AC4 — verified. `site/books.qmd:112-116` states that a mark in the chapter's front matter files one locator, the chapter's page with no fragment, whether the chapter is read from its record or from its source, and the "No fragment" bullet (line 126-127) restates it for both routes; the claim ledger holds the page to 32 claims including the new row (`M52` check, same run). `CHANGELOG.md:48` carries the entry under `## Unreleased` / `### Output` (the only `###` in that section), stating the change and that a single document is untouched.
- AC5 — verified. `tests/run-tests.sh --self-test` exit 0, 1228 checks; `tests/run-tests.sh` exit 0, 654 checks; run one after the other on the branch at `7006e17`, 2026-09-03.
- Consistency gate: `cairn_validate.py` exit 0, every check PASS/OK; no IP/GP text changed (`git diff main...HEAD -- cairn/DESIGN.md` touches Architecture, the recovery section and Known issues only), so `cairn_impact` is skipped; the `generic` profile names no toolchain checks.
- Independent review: three fresh-context lenses. [S] blame-history: zero findings, eleven areas checked consistent with the D-041–D-049 lineage and M064/M070's intent. [S] prior-review: zero findings; archived Review sections M064–M070, KI11 and LESSONS read, GitHub inline-comment probe empty. Diff-bug lens run on Sonnet after four Opus server failures (work log): one finding, F1 below; everything else it checked (filter-table ordering, the Meta walk over nested MetaList/MetaMap/MetaInlines/MetaBlocks, `is_html`/epub gating, the `doc.meta.book` test, `page_locator` plumbing, `fragments.py`, both manifests against the fixture sources, docs claims) it reported correct.
- F1 (diff-bug, ranked first): a mark in a chapter's `title:` is reflected by Quarto into `#quarto-navigation-envelope`, which survives into every page's sidebar and page-navigation markup; the tagging pass declasses only `#quarto-meta-markdown`, so those copies are body marks that mint anchors on every page of the book, and `fragments.py resolve` passes because the anchors are real. Reproduced 2026-09-03 (quarto 1.10.18) in a two-chapter scratch book with `title: "Chapter about [Widget]{.index}"`: on the branch, steady state, `Widget` prints 5 locators — `#qi-mark-1` (page navigation) and `#qi-mark-2` (sidebar) on the index page, `ch1.html` (the title mark's page locator), `ch1.html#qi-mark-1` and `#qi-mark-2` (the chapter's own sidebar and navigation copies); on `main` the same book prints 8, six into `ch1.html`. Pre-existing and reduced by the branch, not introduced by it; no fixture marks a `title:`; the Goal's "once" does not hold for that field. Disposition: at the gate.
- Residue: `M03-AC3/M071` — neither `data-qi-pending` nor `data-qi-meta` reaches any of the 476 captured pages; the `M24` sweep self-test went red on a `data-qi-meta` plant into each page in turn.
