"""Read the filter's own OVERFLOW_JOIN and print it.
...
It asserts exactly one thing: that the source set defines OVERFLOW_JOIN once.
...
PATTERN = r'^local OVERFLOW_JOIN = "(.*)"$'

found = re.findall(PATTERN, filtersrc.text(), re.MULTILINE)
if len(found) != 1:
    print(f'FAIL: M09: expected exactly one OVERFLOW_JOIN definition in the '
