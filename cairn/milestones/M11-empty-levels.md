...
- 2026-08-18: return-1 fix — F1: `sort_levels` now finds `last` in `written` and compares each level's ORIGINAL position (`kept[i]`) against it, so positional filler stays filler; probed against `main`'s filter on one document, both marks on `entry="A!B"` emit `\index{A!zzz@B}` again with no rival-key report. F2: a key written for a dropped level is now reported, counted only within the depth the author wrote and never for an empty sort level lost with an empty entry level. F3: the all-empty fallback returns an EMPTY `kept` rather than nil, so no written key re-aligns onto the level it falls back to. F4: the cross-reference branch says the target went with the entry instead of being suppressed. F5: `arguments()` is brace-matched in both its sites, so an argument carrying an encap is no longer read truncated. F6: the dead `kept`/`depth` locals dropped from `Span`.
- 2026-08-18: return-1 fix — the regression the fixture missed is now in it: `entry="Q!R"` marked twice with `sort="Q!R!S"` and `sort="Q!yyy"`, a shape with NO empty level anywhere, guarded by an exact zero on the rival-key report plus the emitted-argument manifest. Also fixed a hand-list trap I had written myself: the PDF locator check exempted the parent `Birds` by name, so the new parent `Q` failed it; the exemption is now derived from the printed tree (any row the next row is deeper than).
- 2026-08-18: verify slot clean after the fix — `tests/run-tests.sh --self-test`, 182 checks, all passed; README's empty-level pin now carries 11 claims.
