...
Seven rules, each of which warns rather than breaking your build:
...
- **An index no marker names, in an HTML book**, goes at the end of the last
  chapter that places one, and a first render can leave it out. The book's
  last chapter says so once, naming the index, the chapter its section was
  owed to and the chapters whose record that chapter could not read. Give some
  later chapter a marker for that index between two renders and the section
  prints in two chapters instead, which the same chapter reports once by name.
  The [Books](books.qmd) page has the whole of it.
