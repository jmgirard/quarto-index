-- The HTML back-end: the anchors that link an entry back to its mark, the
-- locators they make, and the index section built from the entry tree that
-- `entries.lua` orders and groups.
...
-- The locator a mark adds to its entry in HTML and EPUB, handed to
-- `qi_entries.build_entry_tree`. The fields that name the mark's place here are
-- `anchor`, `href` and `page_locator`; `role` and `paired` are what every
-- back-end reads.
local function html_locator(node, mark)
  ...
  if (mark.anchor or mark.page_locator) and mark.paired ~= "close" then
    ...
    -- than anything a reader wants — the rule the cross-references already
    -- keep. With a MINTED anchor this can never fire, since each
...
local function target_span(root, xref)
  local inlines = qi_entries.literal_inlines(qi_entries.target_text(xref.levels))
...
local function entry_inlines(root, node, name)
  ...
  inlines:insert(pandoc.Span(qi_entries.literal_inlines(node.key),
  ...
  for _, xref in ipairs(node.xrefs) do
    ...
    body:insert(pandoc.Emph(qi_entries.literal_inlines(
...
local function html_index_blocks(marks, taken)
  ...
  for _, name in ipairs(qi_indexes.names()) do
    ...
    if list ~= nil and #list > 0 then
      local root = qi_entries.build_entry_tree(list, html_locator)
      local section_id = mint_section_id(taken, qi_indexes.section_id(name))
      counter = qi_entries.number_entries(root, counter, taken)
      local blocks = pandoc.Blocks({
